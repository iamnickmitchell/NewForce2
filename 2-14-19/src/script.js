...
businesses.forEach(currentBusiness => {
  currentBusiness.orders.forEach(orderArray => {
...
const manufacturingBusinesses = businesses.filter(
  currentBusiness => currentBusiness.companyIndustry === "Manufacturing"
);

manufacturingBusinesses.forEach(business => {
  document.querySelector("#business").innerHTML = `${business.companyName}`;
});
...
const customerFullContact = businesses.map(currentBusiness => {
  return {
    fullName: `${currentBusiness.purchasingAgent.nameFirst}${
      currentBusiness.purchasingAgent.nameLast
    }`,
...
console.log(customerFullContact);
...
let theFox = "";
const sentence = words.reduce((theFox, current) => (theFox += ` ${current}`));
